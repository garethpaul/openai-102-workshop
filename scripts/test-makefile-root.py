#!/usr/bin/env python3
"""Dry-run policy tests for location-independent workshop Make targets."""
...
class MakefileRootTests(unittest.TestCase):
    def run_make(self, *arguments, environment=None):
        with tempfile.TemporaryDirectory(prefix="OpenAI 102's gate ") as directory:
    ...
    def test_all_targets_preserve_spaced_absolute_makefile_path(self):
        targets = (
            "all", "audit", "build", "check", "lint", "lock", "lock-check",
            "lock-upgrade", "root-test", "run", "runtime-check", "smoke",
        ...
        for target in targets:
            for name, arguments, environment in (
                ...
                with self.subTest(target=target, override=name):
                    ...
                    self.assertIn("OpenAI 102's gate ", result.stdout)
